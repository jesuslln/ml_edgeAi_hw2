...
freq = 0.2
time_blackscholes = numpy.linspace(freq,freq*len(df_blackscholes),num=len(df_blackscholes))
...
plt.figure("W_black")
plt.plot(time_blackscholes,df_blackscholes["W"].astype(float), label='Power Consumption')
plt.xlabel('time s')
plt.ylabel('Power W')
plt.legend()
plt.grid(True)
...
print(df_temp.max())
plt.figure("T_black")
plt.plot(time_blackscholes, df_blackscholes['temp6'].astype(float), label='Core max temperature')
plt.xlabel('time s')
plt.ylabel('Temp ºC')
plt.legend()
plt.grid(True)
